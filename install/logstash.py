...
def install(esServer,esUser,esPass):
	...
	if logstashInstalled == False:
		...
		print "  Downloading Logstash 6.3.0"
		os.popen('sudo wget https://artifacts.elastic.co/downloads/logstash/logstash-6.3.0.deb 2>&1').read()
		if not os.path.isfile('logstash-6.3.0.deb'):
			sys.exit('Error downloading logstash')
		if not hashCheck.checkHash('logstash-6.3.0.deb'):
			sys.exit('Error downloading logstash, mismatched file hashes')
		print "  Installing Logstash"
		os.popen('sudo dpkg -i logstash-6.3.0.deb').read()
		print "  Cleaning Up Logstash Installation Files"
		os.remove('logstash-v.deb')
